...
function init() {
    ...
    let gstURL = `wss://quillio-ws-st.herokuapp.com/?sampRate=${context.sampleRate}&keywords=${keywords}`;
    const speechsocket = new WebSocket(gstURL);
    socket = io.connect(`${location.protocol+'//'+location.hostname+(location.port ? ':'+location.port: '')}/meeting`);
